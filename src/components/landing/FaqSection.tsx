...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div 
      className={`border-b last:border-0 transition-colors ${open ? "bg-amber-50/40" : "bg-white"}`}
    ...
    >
      ...
      >
        <span 
          ...
          style={{ color: open ? COLORS.accent : COLORS.primaryText }}
        >
          {q}
        </span>
        <ChevronDown
          size={16}
          className={`flex-shrink-0 mt-0.5 transition-all duration-300 ${open ? "rotate-180" : ""}`}
          style={{ color: open ? COLORS.accent : COLORS.secondaryText }}
        />
      </button>
      <div
        className={`overflow-hidden transition-all duration-300 ease-in-out ${open ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}
      >
        <p className="px-6 pb-5 text-sm leading-relaxed" style={{ color: COLORS.secondaryText }}>
          {a}
        </p>
      </div>
    </div>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-10 text-center">
          <p className="text-sm" style={{ color: COLORS.secondaryText }}>
            ...
            <a
              href="/contact"
              className="underline underline-offset-4 transition-colors font-medium hover:opacity-80"
