...
// import { apiGetProducts, apiCreateOrder , apiUpdateCartItem, apiGetCart} from "@/lib/userApi";
...
// const PAYSTACK_PUBLIC_KEY = process.env.NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY || "pk_test_b8b7d8af804f4f40170a151f7ba3173fc325c591";

// // ── Types ────────────────────────────────────────────────────────────────────
// interface Product {
//   _id: string; productName: string; productCategory: string; productDescription: string;
//   colourCode: string; colourName: string; price: number; stockQuantity: number;
//   coverageInformation: string; productFeatures: string[]; status: string;
// }
...
// // ── Mock products (will be swapped for real cart products later) ──────────────


// const COLOUR_OPTIONS = [
//   "Cream White","Champagne","Silver Mist","Mid Grey","Charcoal",
//   "Bronze","Red Oxide","Midnight Blue","Sage Green","Sand Beige","Pure White","Yellow",
// ];
...
//     console.log('resomo',res)
//     // Safely extract the array depending on your backend layout
//     return res?.cart?.items ?? res?.data ?? [];
//   },
// });

// // 2. Fix the slice logic so it references the extracted array, falling back to MOCK safely
// const products = Array.isArray(fetchedProducts) ? fetchedProducts.slice(0, 3):[]

// console.log('cart products', products)
// // 3. Keep the cart state declaration simple
// const [cart, setCart] = useState<CartItem[]>([]);
// const [cartInitialized, setCartInitialized] = useState(false);

// // 4. Sync the cart state cleanly once data arrives
// useEffect(() => {
//   if (!cartInitialized && fetchedProducts && fetchedProducts.length > 0) {
//     setCart(products.map((p) => ({ 
//       product: p, 
//       quantity: 1, 
//       selectedColour: p.colourName 
//     })));
//     setCartInitialized(true);
//   }
// }, [fetchedProducts, cartInitialized, products]);

// const qc = useQueryClient();

// const updateCartItemQuantity = useMutation({
//   // Accept productId and quantity as properties of a single object
//   mutationFn: async ({ productId, quantity }: { productId: string; quantity: number }) => {
//     console.log('Updating item:', productId, 'with quantity:', quantity);
    
//     // Pass them into your API helper (expecting an object for the body)
...
//   onSuccess: () => {
//     // It's a good idea to invalidate the cart cache here so the UI updates
...
//     setCart((prev) => prev.map((item) =>
//       item.product._id === id ? { ...item, selectedColour: colour } : item
//     ));
//   };
...
//     // Extract reference key safely from the callback argument payload
//     const currentReference = referenceObj?.reference ?? referenceObj?.trxref ?? referenceObj;

//     try {
//       // Direct assignment to the requested backend field: paymentReference
//       await apiCreateOrder({
//         deliveryAddress: delivery.deliveryAddress,
//         state: delivery.state,
//         city: delivery.city,
...
//       setCart([]);
//     } catch (err: unknown) {
//       const msg = err instanceof Error ? err.message : "Payment processed but failed to update order database. Please contact support.";
//       setErrorMsg(msg);
//       toast.error(msg);
//     } finally {
//       setSubmitting(false);
...
//     if (step === 1 && !canGoToPayment)  { toast.error("Please fill in all delivery fields"); return; }
...
//                     ${i < step ? "bg-brand-accent text-brand-black"
//                       : i === step ? "bg-brand-accent text-brand-black ring-4 ring-brand-accent-muted"
...
//                 {isLoading ? (
//                   <div className="py-16 flex justify-center"><Loader size={26} className="animate-spin text-brand-accent" /></div>
//                 ) : cart.length === 0 ? (
...
//                           rounded-xl p-4">
//                         <div className="w-full sm:w-20 h-20 rounded-lg flex-shrink-0 relative overflow-hidden"
...
//                                 <button onClick={() => updateCartItemQuantity.mutate(item.product._id, -1)}
//                                   className="p-1.5 text-brand-mid hover:text-white transition-colors">
//                                   <Minus size={12} />
//                                 </button>
//                                 <span className="text-white text-xs font-semibold w-7 text-center">
//                                   {item.quantity}
//                                 </span>
//                                 <button onClick={() => updateCartItemQuantity.mutate(item.product._id, 1)}
//                                   className="p-1.5 text-brand-mid hover:text-white transition-colors">
...
//                             <span className="text-brand-mid text-xs">{formatCurrency(item.product.price)} each</span>
...
//               <div className="flex flex-col gap-5">
//                 <div className="flex items-center gap-2.5">
//                   <div className="w-9 h-9 rounded-lg bg-brand-accent-muted border border-brand-accent/20
...
//                         </div>
//                         <p className={`text-sm font-semibold ${active ? "text-white" : "text-brand-lt-gray"}`}>{opt.label}</p>
...
//                       <span className="text-brand-mid">Bank Name:</span><span className="text-white">Paint Domain Bank Plc</span>
//                       <span className="text-brand-mid">Account Number:</span><span className="text-white">0123456789</span>
//                       <span className="text-brand-mid">Account Name:</span><span className="text-white">Paint Domain &amp; Primary Interior Builders</span>
//                     </div>
...
//                     {cart.map((item) => (
//                       <div key={item.product._id} className="flex items-center justify-between bg-brand-raised
...
//                             <p className="text-brand-mid text-xs">{item.selectedColour} · ×{item.quantity}</p>
//                           </div>
//                         </div>
//                         <span className="text-brand-accent font-semibold">{formatCurrency(item.product.price * item.quantity)}</span>
//                       </div>
//                     ))}
//                   </div>
//                 </div>
...
//                     <p className="text-brand-mid text-xs mt-1">{delivery.city}, {delivery.state}</p>
//                   </div>
//                 </div>
...
//                     {paymentMethod === "paystack" ? <CreditCard size={15} className="text-brand-accent" /> : <Banknote size={15} className="text-brand-accent" />}
//                     <span className="text-white capitalize">{paymentMethod === "paystack" ? "Paystack" : "Bank Transfer"}</span>
//                   </div>
//                 </div>
...
//               <button onClick={goBack} disabled={step === 0 || submitting}
...
//                 </button>
//               ) : paymentMethod === "bank-transfer" ? (
//                 <button onClick={handleBankTransferSubmit} disabled={submitting || !canSubmit}
//                   className="flex items-center gap-2 bg-brand-accent text-brand-black font-semibold
...
//                   text={submitting ? "Processing..." : "Pay & Place Order"}
...
//                     text-sm disabled:opacity-50 border-none cursor-pointer"
//                 />
//               )}
//             </div>
//           </div>

//           {/* ── Order Summary Sidebar (Restored Section) ──────────────────────── */}
...
//                   Add <span className="text-brand-accent font-medium">{formatCurrency(50000 - subtotal)}</span> more to unlock free delivery!
//                 </p>
//               )}
//             </div>
