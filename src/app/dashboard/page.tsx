...
<div className="grid grid-cols-1 @[400px]:grid-cols-2 sm:grid-cols-4 gap-3">
        {stattistics.map((s) => {
          ...
          return (
            <div key={s.label}
              className={`border rounded-xl p-4 flex flex-col gap-3 transition-transform duration-200 ${s.ring} ${s.bg}`}>
              <div className="flex items-center justify-between">
                <Icon size={17} className={s.text} />
                <span className={`text-[10px] font-medium ${s.ch}`}>{s.change}</span>
              </div>
              <div>
                <p className={`font-bold text-xl sm:text-2xl font-display tracking-tight ${s.text}`}>{s.value}</p>
                <p className="text-brand-mid text-[11px] mt-0.5">{s.label}</p>
              </div>
            </div>
          );
      ...
      <div className="grid lg:grid-cols-3 gap-5">
              ...
              <tbody>
                {orders?.map((o) => (
                  <tr key={o._id}
                    ...
                    <td className="px-5 py-3.5 text-brand-lt-gray text-sm">{o.customerName}</td>
                    <td className="px-5 py-3.5 text-white font-semibold text-sm">{o.paymentStatus}</td>
                    <td className="px-5 py-3.5 text-white font-semibold text-sm">{formatCurrency(o.totalAmount)}</td>
                    <td className="px-5 py-3.5">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-semibold ${statusPill[o.orderStatus]}`}>
                        {o.orderStatus}
        ...
        {/* Quick actions */}
        <div className="bg-brand-card border border-brand-border rounded-xl p-5 h-fit">
          <h3 className="text-white font-semibold text-sm mb-4">Quick Actions</h3>
          <div className="flex flex-col gap-2">
            {quickLinks.map((q) => (
              <Link key={q.label} href={q.href}
