...
// const CATS = ["All","Interior","Exterior","Primer/Undercoat","Textured","Waterproof","Anti-Rust","Floor Coating"];
...
//     queryFn: async () => {
//       try { const res = await apiGetProducts(); return res?.products ?? res?.data ?? []       
//  }
//       catch { return [] }
//     },
...
//   // 1. The mutationFn is where your async API call goes
...
//     console.log('res', res);
//     return res; // Always return the response so onSuccess can use it if needed
//   },
  
//   // 2. These callbacks trigger automatically based on the mutationFn promise lifecycle
//   onSuccess: () => {
//     qc.invalidateQueries({ queryKey: ["portfolio"] }); // double-check if your key should be "cart" or "portfolio"!
...
//                 {filtered.map((p) => {
//                   const inCart = filtered.filter((item) => item._id === p._id);
//                   return (
//                     <div key={p._id}
...
//                         <button
//                         //  onClick={() => setCart((prev) => prev.includes(p._id) ? prev : [...prev, p._id])
...
//                               : "bg-brand-accent text-brand-black hover:bg-brand-accent-lt"
//                           }`}>
//                           {/* {inCart ? <CheckCircle size={14} /> : <ShoppingCart size={14} />} */}
//                               {inCart.length > 0 ? "Added to Cart" : "Add to Cart"}
//                         </button>
//                       </div>
//                     </div>
//                   );
