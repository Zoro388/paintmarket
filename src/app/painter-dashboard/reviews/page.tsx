...
// import { painterReviews } from "@/lib/painterApi";
...
//     queryKey: ["painter-reviews"],
//     queryFn: async () => {
//       const res = await painterReviews ();
//       console.log('res', res);
...
export default function PainterReviewsPage() {
  ...
  // Only show reviews intended to be public/visible to the painter
  ...
  if (visibleReviews.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-24 text-center max-w-md mx-auto px-4">
        <div className="p-4 rounded-full bg-brand-card border border-brand-mid/20">
          <MessageSquare size={36} className="text-brand-mid" />
        </div>
        <h3 className="text-white font-semibold text-lg">No reviews yet</h3>
        <p className="text-brand-mid text-sm">
          When customers leave feedback on your profile, their thoughts and ratings will appear right here.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 px-4  w-full py-6">
      <div>
        <h1 className="font-display text-2xl font-bold text-white">My Reviews</h1>
        <p className="text-brand-mid text-sm mt-1">
          See what clients are saying about your painting services
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {visibleReviews.map((review) => (
          <div
            key={review._id}
            className="bg-brand-card border border-brand-mid/20 rounded-xl p-5 shadow-sm hover:border-brand-mid/40 transition-colors flex flex-col gap-3"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-brand-subtle text-xs font-mono mt-0.5">
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                <span className="text-[11px] font-bold text-brand-accent bg-brand-black/40 px-2 py-0.5 rounded border border-brand-mid/10">
                  {review.rating} / 5
                </span>
              </div>
            </div>
            ...
            {review.review ? (
              <p className="text-brand-lt-gray text-sm leading-relaxed whitespace-pre-wrap pt-1">
                "{review.review}"
              </p>
            ) : (
              <p className="text-brand-subtle italic text-xs pt-1">
                Left a rating score without a written comment.
              </p>
            )}
          </div>
        ))}
